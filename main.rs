...
fn main() {
    ...
    let mut thread_sums: Vec<usize> = Vec::new();
    threaded_map_data(num_partitions, &partitions, &mut thread_sums);
    // 5. Prints information about the intermediate sums
    println!("Intermediate sums = {:?}", thread_sums);        //print vector
    // 5. Calls reduce_data to process the intermediate sums
    let thread_sum = reduce_data(&thread_sums);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut start = 0;
    let mut end;
    let mut mod_val = v.len()%num_partitions;     //allows the loop to have 1 more item to partition
    
    //loop through number of partitions
    for i in 0..num_partitions{
        let mut y: Vec<usize> = Vec::new();
        end = if mod_val > 0 {start + range + 1} else {start + range};
        for x in start..end{
            y.push(v[x]);       //push each value of v into partition
        }
        xs.push(y);
        mod_val -= if mod_val > 0 {1} else {0};
        if i != num_partitions -1 {start = end}
    }
    return xs       //vector that contains vectors of integers
}
fn threaded_map_data(num_partitions: usize, v: &Vec<Vec<usize>>, intermediate_sums :&mut Vec<usize>){
    let mut results : Vec<thread::JoinHandle<_>> = Vec::new();
    for i in 0..num_partitions {
        let arg = v[i].clone();     // Creates a clone for each vector processed by the threads
        results.push(thread::spawn(move || map_data(&arg)));        //create thread
    }
    for _ in 0..num_partitions {    //source: https://stackoverflow.com/questions/29932503/what-is-the-idiomatic-way-to-write-a-for-loop-without-using-the-iterator-value
        let thread = results.remove(0);
        let sum = thread.join().unwrap();   //join to end threads
        intermediate_sums.push(sum);        //push intermediate results sums
    }
}